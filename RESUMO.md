...
| **Fase 3 — Qolari Chat + Angola** | Motor de **subscrições** (Stripe subscription + plafond/rate-limit/throttling) + **Chat UI browser** (histórico sync, tabela de mensagens) + providers oficiais múltiplos por config + **geo-pricing (IP + país)** + **AOA/Multicaixa Express (EMIS)** + EuPago se procura PT | ⬜ |
