...
## Important Notes
- Files in `src-read-only/` are auto-generated by QolariDSL. They should ONLY change when corresponding `spec/` YAML files change. If `src-read-only/` files changed WITHOUT spec changes, flag as suspicious.
