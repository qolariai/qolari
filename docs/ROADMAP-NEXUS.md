...
>
> **✅ PIVÔ EXECUTADO (06-08-2026):** abstração de providers (`config/ai_providers.php` + `AiProviderResolver`), proxy provider-aware, `SyncModelCosts`/`ReconcileStreamUsage` adaptados, seeder com motores DeepSeek direto + NVIDIA free, admin Model Costs. **42/42 testes.** Registado em `docs/MODEL_CHANGELOG.md` (entrada 2026-08-06). **✅ FECHO EM PRODUÇÃO (07-08-2026):** deploy backend+frontend (`api.qolari.com/api/v1/*`, shim nginx `/v1/*` p/ clientes OpenAI/extensão), key NVIDIA no `.env` do servidor validada com benchmark `nexus-low` **16/16**, Stripe **test** + webhook criado (`/api/v1/webhooks/stripe`), 73/73 testes. Pendente: key oficial DeepSeek (compra) + benchmark `nexus-high`, Stripe **live** (dados da empresa).
>
> **✅ FASE 2.5 — SPIKE TÉCNICO CONCLUÍDO (07-08-2026):** fork do Cline em `extensao/` com diff mínimo (13 ficheiros, edições marcadas `// Qolari fork:`): rebrand `qolari-code`, provider único bloqueado ao proxy (`https://api.qolari.com/v1`, modelo default `nexus-high` livre-texto), PostHog desligado. Build verde → `extensao/apps/vscode/dist/qolari-code.vsix` (11.9 MB). **Nota (07-08):** a extensão aponta para `/v1` (sem prefixo `/api`) — funciona em produção via shim nginx, mas normalizar para `/api/v1` (consistente com o IDE) no próximo rebuild. Pendente: revisão legal (ToS/Apache), contas publisher (Marketplace + Open VSX), branding residual, publicar, commitar o spike (está só no working tree).
>
> **✅ FASE 3 (BACKEND) CONCLUÍDA (07-08-2026):** motor de subscrições + Chat backend. Tabelas `subscription_plans(+prices)`, `subscriptions`, `chat_conversations`, `chat_messages`; `SubscriptionService` (plafond de tokens por período, throttling, 402/429 white-label); Stripe `mode: subscription` + webhooks (`checkout.session.completed`, `customer.subscription.updated/deleted`, `invoice.payment_failed`) idempotentes via `payment_events`; Chat API com histórico persistido e streaming, billing **na subscrição** (`UsageMeter billTo='subscription'` — wallets intactas); admin Filament Plans/Subscriptions. **71/71 testes.** **✅ Chat UI (mesma data):** página `/dashboard/chat` (conversas, streaming SSE, barra de plafond, throttling, paywall 402/429) + planos de subscrição na pricing + endpoint público `GET /v1/subscription-plans`. **73/73 testes, build Next.js verde.** Pendente: geo-pricing, Angola (AOA/Multicaixa), criar planos no admin (sem seeding por design).
>
> **✅ ANGOLA / APPYPAY — CODE (08-08-2026):** gateway **AppyPay** integrada para créditos Code em **AOA** via Multicaixa Express (GPO). `AppyPayService` (OAuth `client_credentials` + `POST /charges`, modo async com callback), webhook assinado HMAC-SHA256 (`POST /api/v1/webhooks/appypay`, idempotente via `payment_events`), fulfillment partilhado extraído para `OrderFulfillmentService` (StripeService delega), coluna `orders.gateway_reference`, credenciais `appypay_*` no admin (Settings) com fallback `.env`, checkout aceita `currency=AOA` + `phone` (validação/normalização 9XXXXXXXX), pricing page com moeda AOA e fluxo de telemóvel + polling ao estado da order. **82/82 testes (7 novos `AppyPayCheckoutTest`), tsc verde.** Pendente: credenciais AppyPay (contrato com banco de apoio + EMIS), preços AOA dos pacotes no admin, subscrições Chat em AOA (débito direto SDD — a avaliar).
>
> **✅ GEO-PRICING POR IP (08-08-2026):** `GET /v1/geo` público (`GeoService` → ipapi.co, cache 24h, falha silenciosa) sugere AOA para IPs angolanos; pricing page aplica hierarquia país da conta (AO→AOA) → `preferred_currency` → IP, sem sobrepor escolha manual; `preferred_currency` passa a aceitar AOA (register/profile + settings). **88/88 testes (6 novos `GeoPricingTest`), tsc verde.** Smoke tests do mesmo dia: instalador Windows OK (instalação silenciosa + arranque + UI funcional, histórico de chat presente — app usada com sucesso); VSIX íntegro (manifest/NOTICE/dist OK, hash = cópia alojada). Observações: productName do instalador ainda "Qolari IDE Dev" e título da janela "OpenCode" (branding residual a corrigir no próximo build).
>
