...
atlasDB =
  B.defaultDbSettings
    `B.withDbModification` B.dbModification
      { accessMatrix = BeamAM.accessMatrixTable,
        merchant = BeamM.merchantTable,
        merchantAccess = BeamMA.merchantAccessTable,
        person = BeamP.personTable,
        registrationToken = BeamRT.registrationTokenTable,
        role = BeamR.roleTable,
        transaction = BeamT.transactionTable
      }
