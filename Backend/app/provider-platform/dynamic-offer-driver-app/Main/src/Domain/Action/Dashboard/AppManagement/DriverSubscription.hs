  ...
  withLogTag ("personId_" <> personId.getId) $ do
    case channel of
      SMS -> do
        ...
        (mbSender, message, templateId, messageType) <- MessageBuilder.buildGenericMessage merchantOpCityId mkey mbVehicleCategory MessageBuilder.BuildGenericMessageReq {}
  ...
  where
    ...
    getManualDues personId = do
      sum . map (\dueInvoice -> SLDriverFee.roundToHalf dueInvoice.currency (dueInvoice.govtCharges + dueInvoice.platformFee.fee + dueInvoice.platformFee.cgst + dueInvoice.platformFee.sgst))
        <$> QDF.findAllFeeByTypeServiceStatusAndDriver YATRI_SUBSCRIPTION personId [RECURRING_INVOICE] [PAYMENT_OVERDUE]
...
postDriverSubscriptionUpdateDriverFeeAndInvoiceInfo merchantShortId opCity driverId serviceName' DriverSubscription.SubscriptionDriverFeesAndInvoicesToUpdate {..} = do
  ...
  where
    upsertVendorFees :: UTCTime -> Maybe [DVF.VendorFee] -> Id DMC.MerchantOperatingCity -> Flow [DVF.VendorFee]
    upsertVendorFees now vendorFeesToUpsert merchantOpCityId = do
            ...
            ( \vendorFee -> do
                ...
                case mbVendorFeeExisting of
                  Just vendorFeeExisting -> do
                    ...
                    QVF.updateVendorFee merchantOpCityId vendorFeeWithTimestamp
