...
issueBreachMitigation issueType transporterConfig driverInfo = when (isJust transporterConfig.issueBreachConfig && not driverInfo.blocked) $ do
  ...
  where
    ...
    tryBlockDriver :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r, CoreMetrics m, HasLocationService m r, JobCreator r m, HasShortDurationRetryCfg r c, Redis.HedisLTSFlowEnv r) => Int -> [ServiceTierType] -> IssueBreachBlockType -> Int -> Int -> Int -> Int -> Int -> Maybe UTCTime -> UTCTime -> BlockReasonFlag -> m Bool
