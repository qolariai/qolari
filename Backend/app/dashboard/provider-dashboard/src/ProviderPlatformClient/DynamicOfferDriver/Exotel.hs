...
module ProviderPlatformClient.DynamicOfferDriver.Exotel
  ( callDynamicOfferDriverAppExotelApi,
  )
where

import qualified "dynamic-offer-driver-app" API.Dashboard.Exotel as BPP
...
mkDynamicOfferDriverAppExotelAPIs token = do
  ExotelAPIs {..}
  where
    exotelHeartbeat = Euler.client (Proxy :: Proxy BPP.API) token
...
callDynamicOfferDriverAppExotelApi ::
  ...
  (ExotelAPIs -> b) ->
  c
callDynamicOfferDriverAppExotelApi = callServerAPI @_ @m @r DRIVER_OFFER_BPP_MANAGEMENT mkDynamicOfferDriverAppExotelAPIs "callDynamicOfferDriverAppExotelApi"
