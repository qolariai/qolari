...
getFleetOwnerId memberPersonId mbFleetOwnerId = Client.callFleetAPI (skipMerchantCityAccessCheck (ShortId "no_required")) (City.City "AnyCity") (.driverDSL.getDriverDashboardInternalHelperGetFleetOwnerId) mbFleetOwnerId memberPersonId
...
getFleetOwnerIds memberPersonId mbFleetOwnerId = do
  ...
  case mbCachedFleetOwnerIds of
    ...
    Nothing -> do
      fleetOwnerIds <- Client.callFleetAPI (skipMerchantCityAccessCheck (ShortId "no_required")) (City.City "AnyCity") (.driverDSL.getDriverDashboardInternalHelperGetFleetOwnerIds) mbFleetOwnerId memberPersonId
...
postDriverFleetScheduledBookingCancel :: (Kernel.Types.Id.ShortId DM.Merchant -> City.City -> ApiTokenInfo -> Common.CancelScheduledBookingReq -> Environment.Flow Kernel.Types.APISuccess.APISuccess)
postDriverFleetScheduledBookingCancel merchantShortId opCity apiTokenInfo req = do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  transaction <- buildTransaction apiTokenInfo Nothing (Just req)
  T.withTransactionStoring transaction $ do
    let fleetOwnerId = apiTokenInfo.personId.getId
    Client.callFleetAPI checkedMerchantId opCity (.driverDSL.postDriverFleetScheduledBookingCancel) fleetOwnerId req
...
postDriverFleetScheduledBookingReassign merchantShortId opCity apiTokenInfo req = do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  transaction <- buildTransaction apiTokenInfo (Just req.newDriverId) (Just req)
  T.withTransactionStoring transaction $ do
    let fleetOwnerId = apiTokenInfo.personId.getId
