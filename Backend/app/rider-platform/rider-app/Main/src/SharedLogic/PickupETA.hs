...
getPickupETAFromModel timeDiffFromUtc speedInMps distanceInMeters mbVersion merchantOperatingCityId = do
  ...
  if null allLogics
    then do
      ...
      response <- withTryCatch "runLogics:getPickupETAFromModel" $ LYDL.runLogicsWithDebugLog LYDL.Rider (cast merchantOperatingCityId) LYT.PICKUP_ETA_CALCULATION Nothing allLogics inputData
