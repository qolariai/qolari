...
upsertPersonAndGetFare partnerOrg mbIntegratedBPPConfigId req = withFlowHandlerAPI . withLogTag $ do
  ...
  Log.withLogTag ("FRFS:GetFare:PersonId:" <> personId.getId) $ do
    ...
    res <- DFRFSTicketService.postFrfsSearchHandler (personId, merchantId) merchantOperatingCity integratedBPPConfig frfsVehicleType frfsSearchReq frfsRouteDetails req.partnerOrgTransactionId (Just partnerOrg.orgId) Nothing Nothing (\_ -> pure ()) [] [] True Nothing -- the journey leg upsert function is not required here
