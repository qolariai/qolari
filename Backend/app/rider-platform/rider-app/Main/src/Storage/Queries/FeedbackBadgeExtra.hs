...
findFeedbackBadgeForRider :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r) => Id.Id Person.Person -> Text -> m (Maybe Domain.FeedbackBadge)
findFeedbackBadgeForRider riderId badge =
  findOneWithKV [Se.And [Se.Is Beam.riderId $ Se.Eq (Id.getId riderId), Se.Is Beam.badge $ Se.Eq badge]]

findFeedbackBadgeByKey :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r) => Id.Id Person.Person -> Text -> m (Maybe Domain.FeedbackBadge)
findFeedbackBadgeByKey riderId badgeKey =
  findOneWithKV [Se.And [Se.Is Beam.riderId $ Se.Eq (Id.getId riderId), Se.Is Beam.badgeKey $ Se.Eq (Just badgeKey)]]
...
updateFeedbackBadge feedbackBadge newBadgeCount = do
  ...
  updateWithKV
    ...
    [Se.And [Se.Is Beam.id $ Se.Eq (Id.getId feedbackBadge.id), Se.Is Beam.riderId $ Se.Eq (Id.getId feedbackBadge.riderId)]]
