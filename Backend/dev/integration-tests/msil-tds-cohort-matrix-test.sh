...
#   │ DRIVER         │ INDIVIDUAL   │ aadhaar_to_other    │   0.05   │ driver_information       │  (strict-linkage)
...
#   │ FLEET_OWNER    │ INDIVIDUAL   │ aadhaar_to_other    │   0.05   │ fleet_owner_information  │  (strict-linkage)
...
#   │ FLEET_BUSINESS │ INDIVIDUAL   │ aadhaar_to_other    │   0.05   │ fleet_owner_information  │  (strict-linkage)
...
#   2. transporter_config.tax_config for MSIL MOC has rideGst/subscriptionGst and
#      the TdsConfig cohort fields individualLinked/individualNotLinked/
#      businessTds plus invalidPanTdsRate, each { rate, thresholdAmount }.
#      individualNotLinked present = spec-mode opt-in;
#      thresholdAmount carries the 194O threshold (e.g. 500000).
...
RATE_INVALID="0.20"
RATE_BUSINESS="0.001"   # businessTds cohort (selectTds routes any valid business PAN here, role-agnostic)
...
run_case D3 DRIVER INDIVIDUAL VALID 'false' "$OTHER_PAN_MSG"  "$OTHER_PAN_STATUS"  "$RATE_NOT_LINKED" "Individual + Aadhaar-to-other-PAN → 5% (strict-linkage: only PAN_AADHAAR_LINKED counts)"
run_case D4 DRIVER BUSINESS   VALID 'true'  "$LINKED_MSG"     "$LINKED_STATUS"     "$RATE_BUSINESS"   "Business PAN on DRIVER → businessTds 0.1% (role-agnostic; barred at onboarding so shouldn't occur)"
...
run_case F3 FLEET_OWNER INDIVIDUAL VALID 'false' "$OTHER_PAN_MSG"  "$OTHER_PAN_STATUS"  "$RATE_NOT_LINKED" "Individual + Aadhaar-to-other-PAN → 5% (strict-linkage)"
run_case F4 FLEET_OWNER BUSINESS   VALID 'true'  "$LINKED_MSG"     "$LINKED_STATUS"     "$RATE_BUSINESS"   "Business PAN on Individual Fleet → businessTds 0.1% (role-agnostic; barred at onboarding so shouldn't occur)"
...
run_case B3 FLEET_BUSINESS INDIVIDUAL VALID 'false' "$OTHER_PAN_MSG"  "$OTHER_PAN_STATUS"  "$RATE_NOT_LINKED" "Individual + Aadhaar-to-other-PAN → 5% (strict-linkage)"
run_case B4 FLEET_BUSINESS BUSINESS   VALID 'true'  "$LINKED_MSG"     "$LINKED_STATUS"     "$RATE_BUSINESS"   "Business PAN on Business Fleet → businessTds 0.1% (legitimate cohort)"
...
# N1: a non-MSIL merchant (individualNotLinked absent from its tax_config) should
...
NONMSIL_ROW=$(psql_q "
  ...
  WHERE m.short_id <> '${MSIL_MERCHANT_SHORT_ID}'
    AND (tc.tax_config -> 'individualNotLinked') IS NULL
  LIMIT 1;
")
if [[ -z "$NONMSIL_ROW" ]]; then
  echo -e "  ${YELLOW}SKIP${NC} [N1] no non-MSIL merchant with individualNotLinked absent found in local DB"
else
