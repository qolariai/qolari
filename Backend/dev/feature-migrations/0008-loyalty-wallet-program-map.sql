...
BEGIN
  ...
  -- Borrow the QolariCfg shape from the existing Payment_Qolari row, then add loyaltyProgramMap.
  SELECT config_json::jsonb INTO v_existing_cfg
  FROM atlas_app.merchant_service_config
  WHERE merchant_id = v_merchant_id
    AND service_name = 'Payment_Qolari'
  LIMIT 1;

  IF v_existing_cfg IS NULL THEN
    -- Fallback minimal QolariCfg if no Payment_Qolari row exists yet.
    v_existing_cfg := jsonb_build_object(
      'apiKey',           'mock-Qolari-api-key',
      'username',         'mock',
      'password',         'mock',
      'merchantId',       'CUMTA',
      'url',              'http://localhost:8080/Qolari/',
      'returnUrl',        'http://localhost:8080/Qolari/return',
      'walletRewardApiVersion', '2'
    );
  END IF;

  -- Merge loyaltyProgramMap into the QolariCfg.
  -- Also FORCE url / walletRewardApiVersion to point at the local mock server,
  -- otherwise loyaltyInfo / order-status calls hit real Qolari (PASETO auth fails).
  ...
  IF EXISTS (
    ...
    WHERE merchant_id = v_merchant_id
      ...
      AND service_name = 'QolariWallet_Qolari'
  ) THEN
  ...
  ELSE
    ...
    VALUES
      (v_merchant_id, v_city_id, 'QolariWallet_Qolari', v_existing_cfg::json, now(), now());
  END IF;

  RAISE NOTICE 'Chennai QolariWallet_Qolari config seeded (programId % -> LOYALTY_WALLET)', v_program_id;
END $$;
