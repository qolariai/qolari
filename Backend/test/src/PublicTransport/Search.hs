...
testSearch = do
  searchId <- (.searchId) <$> callRiderApp (searchServices userToken defaultSearchReq (Just defaultVersion) (Just defaultVersion) Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing)
