...
- **Accounting equation** the recipe enforces:
  ```
  planRideCredit == consumed + remaining
                    ────────   ─────────
                    settled     wallet slice from
                    debits +    getSubscriptionRemainingAvailableBalance
                    expiries    (0 for terminal subs)
  ```
- **Discovery** — every ACTIVE prepaid sub in the merchant operating city is reconciled each chunk, regardless of whether it had ledger activity. Uses `findAllActiveInMerchantOpCityAndServiceName` (one query, no per-owner iteration). Rationale: chunking by `purchaseTimestamp` would miss subs bought weeks ago that are still consuming; chunking by ledger activity would miss ACTIVE subs that stopped transacting when they shouldn't have. All-active covers both cases and matches the MSIL "all subscriptions" requirement.

- **Expected** (source): `subscription.planRideCredit` — money credit granted at purchase.
- **Actual** (target): one composite `TargetRecord` per sub with `tgtAmount = consumed + remaining`.
  - `consumed = sum(settled RideSubscriptionDebit for the sub's rides) + sum(settled ExpiryCreditTransfer for the sub)`
  - `remaining = getSubscriptionRemainingAvailableBalance sub consumed` — `Just` for ACTIVE (FIFO wallet slice), `Nothing` for terminal (interpreted as 0).
- **Status mapping**: `PENDING` → `InFlight` (never enters source list — discovery is ACTIVE-only). `ACTIVE` → `Settled` (matched continuously via `remaining`). `EXPIRED` / `EXHAUSTED` / `FAILED` never enter the source list either — a sub that just transitioned to terminal status in this chunk keeps its last MATCHED entry until the sweep closes it as `AGED_OUT`.
- `maxOpenAge = 90 days`. Only unresolved mismatches remain OPEN — MATCHED entries close immediately since sources are always `Settled`.
- **Ride fetch is per-sub via `findAllBySubscriptionPurchaseId`** in a `forM` loop; the `subscriptionPurchaseIds` column on `Ride` is a Postgres array so a proper bulk fetch needs an array-intersection query Beam doesn't expose ergonomically. TODO comment inline. Fine at current chunk volumes.
- **Orphan check** targets `ExpiryCreditTransfer` ledger entries whose referenced subscription doesn't exist anywhere in `subscription_purchase`. Chunk-boundary artifacts (sub was purchased in a prior chunk, still exists) are filtered out via a bulk `findByIds` lookup so only phantom-subscription cases surface as `MISSING_IN_SOURCE`. Ride-debit orphans aren't covered — `reference_id` is a booking id, not a sub id.
