...
spec = describe "BecknV2.OnDemand.Types" $ do
  ...
  describe "Descriptor (existing type, unchanged)" $ do
    ...
    it "round-trips with Nothing fields" $ do
      let desc = Spec.Descriptor {descriptorCode = Nothing, descriptorName = Nothing, descriptorShortDesc = Nothing, descriptorLongDesc = Nothing}
      roundTrip desc
  ...
  describe "Authorization (existing type, unchanged)" $ do
    ...
    it "round-trips with Nothing fields" $ do
      let auth = Spec.Authorization {authorizationType = Nothing, authorizationToken = Nothing, authorizationStatus = Nothing}
      roundTrip auth
  ...
  describe "spec switching: v2.0.0 → v2.1.0" $ do
    ...
    it "v2.0.0 Fulfillment JSON with no energy_type in vehicle parses cleanly" $ do
      let json =
            ...
            \\"state\": {\"descriptor\": {\"code\": \"RIDE_ASSIGNED\"}}}" ::
              BL.ByteString
    ...
    it "v2.0.0 Context with version 2.0.0 parses cleanly" $ do
      let json =
            ...
            \\"version\": \"2.0.0\", \"bap_id\": \"bap.test.com\"}" ::
              BL.ByteString
  ...
  describe "spec switching: v2.1.0 → v2.0.0 (forward compat)" $ do
    ...
    it "v2.1.0 Context version 2.1.0 parses cleanly" $ do
      let json =
            ...
            \\"version\": \"2.1.0\", \"bpp_id\": \"bpp.test.com\"}" ::
              BL.ByteString
      let decoded = decode json :: Maybe Spec.Context
      decoded `shouldSatisfy` isJust
    ...
    it "v2.1.0 Order with item cancellation_terms round-trips" $ do
      let cancelTerm =
            Spec.CancellationTerm
              { cancellationTermCancellationFee = Nothing,
                cancellationTermFulfillmentState = Just (Spec.FulfillmentState {fulfillmentStateDescriptor = Just (Spec.Descriptor {descriptorCode = Just "RIDE_CONFIRMED", descriptorName = Nothing, descriptorShortDesc = Nothing, descriptorLongDesc = Nothing})}),
                cancellationTermReasonRequired = Just False
              }
  ...
  describe "spec switching: mixed payloads" $ do
    ...
    it "Provider with v2.0.0 fields + v2.1.0 categories coexist" $ do
      let json =
            ...
            \\"categories\": [{\"id\": \"ON_DEMAND_TRIP\", \"descriptor\": {\"code\": \"ON_DEMAND_TRIP\"}}]}" ::
              BL.ByteString
      let decoded = decode json :: Maybe Spec.Provider
      decoded `shouldSatisfy` isJust
    ...
    it "Fulfillment with v2.1.0 vehicle energy_type + existing fields" $ do
      let json =
            ...
            \\"stops\": [{\"type\": \"START\", \"instructions\": {\"name\": \"Take Gate A\"}}]}" ::
              BL.ByteString
      let decoded = decode json :: Maybe Spec.Fulfillment
      decoded `shouldSatisfy` isJust
      let ff = fromJust decoded
  ...
  -- ================================================================
  -- Golden contract tests (Section I): v2.1.0 realistic payloads
  -- ================================================================

  describe "golden: on-demand trip (baseline)" $ do
    it "on_search response with categories and items round-trips" $ do
      let provider =
            minimalProvider
              { Spec.providerId = Just "provider_1",
                Spec.providerCategories = Just [sampleCategory],
                Spec.providerItems =
                  Just
                    [ minimalItem
                        { Spec.itemId = Just "item_1",
                          Spec.itemCategoryIds = Just ["ON_DEMAND_TRIP"],
                          Spec.itemPrice = Just samplePrice
                        }
                    ]
              }
      roundTrip provider
      (Spec.providerCategories provider >>= \xs -> Just (length xs)) `shouldBe` Just 1

    it "confirm order with fulfillment and payment round-trips" $ do
      let ff =
            minimalFulfillment
              { Spec.fulfillmentId = Just "ff_1",
                Spec.fulfillmentType = Just "DELIVERY",
                Spec.fulfillmentState =
                  Just
                    Spec.FulfillmentState
                      { fulfillmentStateDescriptor =
                          Just Spec.Descriptor {descriptorCode = Just "RIDE_CONFIRMED", descriptorName = Nothing, descriptorShortDesc = Nothing, descriptorLongDesc = Nothing}
                      }
              }
      let order =
            minimalOrder
              { Spec.orderId = Just "order_1",
                Spec.orderStatus = Just "ACTIVE",
                Spec.orderFulfillments = Just [ff],
                Spec.orderPayments = Just [samplePayment]
              }
      roundTrip order

  describe "golden: payment confirmation flow (BPP collecting)" $ do
    it "payment with NOT-PAID status preserved" $ do
      let payment =
            minimalPayment
              { Spec.paymentCollectedBy = Just "BPP",
                Spec.paymentType = Just "ON-FULFILLMENT",
                Spec.paymentStatus = Just "NOT-PAID"
              }
      let decoded = decode (encode payment) :: Maybe Spec.Payment
      (decoded >>= Spec.paymentStatus) `shouldBe` Just "NOT-PAID"
      (decoded >>= Spec.paymentCollectedBy) `shouldBe` Just "BPP"

    it "payment transition to PAID preserved" $ do
      let payment =
            minimalPayment
              { Spec.paymentCollectedBy = Just "BPP",
                Spec.paymentType = Just "ON-FULFILLMENT",
                Spec.paymentStatus = Just "PAID"
              }
      let decoded = decode (encode payment) :: Maybe Spec.Payment
      (decoded >>= Spec.paymentStatus) `shouldBe` Just "PAID"

  describe "golden: scheduled trip with timestamped stop" $ do
    it "stop with time.timestamp and time.duration round-trips" $ do
      let stop =
            minimalStop
              { Spec.stopType = Just "START",
                Spec.stopTime =
                  Just
                    Spec.Time
                      { timeTimestamp = Nothing, -- UTCTimeRFC3339 not easily constructible in tests
                        timeDuration = Just "PT10M"
                      },
                Spec.stopInstructions = Just sampleDescriptor
              }
      roundTrip stop

    it "scheduled stop JSON has correct keys" $ do
      let stop =
            minimalStop
              { Spec.stopType = Just "START",
                Spec.stopTime = Just Spec.Time {timeTimestamp = Nothing, timeDuration = Just "PT10M"}
              }
      stop `hasJsonKey` "type"
      stop `hasJsonKey` "time"

  describe "golden: quote update (tipping) breakup" $ do
    it "quotation with BUYER_ADDITIONAL_AMOUNT breakup round-trips" $ do
      let tipBreakup =
            Spec.QuotationBreakupInner
              { quotationBreakupInnerTitle = Just "BUYER_ADDITIONAL_AMOUNT",
      ...
      let baseFareBreakup =
            Spec.QuotationBreakupInner
              { quotationBreakupInnerTitle = Just "BASE_FARE",
                quotationBreakupInnerPrice = Just samplePrice
              }
      let quotation =
            Spec.Quotation
              { quotationBreakup = Just [baseFareBreakup, tipBreakup],
                quotationPrice = Just samplePrice,
                quotationTtl = Nothing
              }
      roundTrip quotation
      let decoded = decode (encode quotation) :: Maybe Spec.Quotation
      (decoded >>= Spec.quotationBreakup >>= \xs -> Just (length xs)) `shouldBe` Just 2

    it "BUYER_ADDITIONAL_AMOUNT breakup has correct JSON keys" $ do
      let breakup =
            Spec.QuotationBreakupInner
              { quotationBreakupInnerTitle = Just "BUYER_ADDITIONAL_AMOUNT",
                quotationBreakupInnerPrice = Just samplePrice
              }
      breakup `hasJsonKey` "title"
      breakup `hasJsonKey` "price"

  describe "golden: error code 90203 (driver not assigned)" $ do
    it "Error with code 90203 round-trips" $ do
      let err =
            Spec.Error
              { errorCode = Just "90203",
                errorMessage = Just "Driver not assigned",
                errorPaths = Nothing
              }
      roundTrip err

    it "Error serializes to correct JSON keys" $ do
      let err =
            Spec.Error
              { errorCode = Just "90203",
                errorMessage = Just "Driver not assigned",
      ...
      err `hasJsonKey` "code"
      err `hasJsonKey` "message"
      err `lacksJsonKey` "errorCode"
      err `lacksJsonKey` "paths" -- omitNothingFields
    it "v2.0.0 error format still parses (no paths field)" $ do
      let json = "{\"code\": \"90203\", \"message\": \"Driver not assigned\"}" :: BL.ByteString
      let decoded = decode json :: Maybe Spec.Error
      decoded `shouldSatisfy` isJust
      Spec.errorCode (fromJust decoded) `shouldBe` Just "90203"

  describe "golden: category codes for product variants" $ do
    it "ON_DEMAND_TRIP category round-trips" $ do
      roundTrip (Spec.Category {categoryDescriptor = Just (Spec.Descriptor {descriptorCode = Just "ON_DEMAND_TRIP", descriptorName = Just "ON_DEMAND_TRIP", descriptorShortDesc = Nothing, descriptorLongDesc = Nothing}), categoryId = Just "ON_DEMAND_TRIP"})

    it "ON_DEMAND_RENTAL category round-trips" $ do
      roundTrip (Spec.Category {categoryDescriptor = Just (Spec.Descriptor {descriptorCode = Just "ON_DEMAND_RENTAL", descriptorName = Just "ON_DEMAND_RENTAL", descriptorShortDesc = Nothing, descriptorLongDesc = Nothing}), categoryId = Just "ON_DEMAND_RENTAL"})

    it "SCHEDULED_TRIP item with category_ids round-trips" $ do
      let item = minimalItem {Spec.itemId = Just "item_sched", Spec.itemCategoryIds = Just ["SCHEDULED_TRIP"]}
      roundTrip item

    it "multiple category_ids on single item round-trips" $ do
      let item = minimalItem {Spec.itemCategoryIds = Just ["ON_DEMAND_TRIP", "ON_DEMAND_RENTAL"]}
      let decoded = decode (encode item) :: Maybe Spec.Item
      Spec.itemCategoryIds (fromJust decoded) `shouldBe` Just ["ON_DEMAND_TRIP", "ON_DEMAND_RENTAL"]

  describe "golden: rental search with only START stop" $ do
    it "fulfillment with single START stop (no END) round-trips" $ do
      let ff =
            minimalFulfillment
              { Spec.fulfillmentType = Just "RENTAL",
                Spec.fulfillmentStops =
                  Just
                    [ minimalStop
                        { Spec.stopType = Just "START",
                          Spec.stopTime = Just Spec.Time {timeTimestamp = Nothing, timeDuration = Nothing}
                        }
                    ]
              }
      roundTrip ff
      let decoded = decode (encode ff) :: Maybe Spec.Fulfillment
      (decoded >>= Spec.fulfillmentStops >>= \xs -> Just (length xs)) `shouldBe` Just 1

    it "rental fulfillment parses from JSON with only START stop" $ do
      let json =
            "{\"type\": \"RENTAL\", \
            \\"stops\": [{\"type\": \"START\"}]}" ::
              BL.ByteString
      let decoded = decode json :: Maybe Spec.Fulfillment
      decoded `shouldSatisfy` isJust
      Spec.fulfillmentType (fromJust decoded) `shouldBe` Just "RENTAL"
      (Spec.fulfillmentStops (fromJust decoded) >>= \xs -> Just (length xs)) `shouldBe` Just 1

  describe "golden: tags compatibility 2.0.0 vs 2.1.0 layout" $ do
    it "v2.0.0 payment with BUYER_FINDER_FEES tag group in JSON" $ do
      let json =
            "{\"collected_by\": \"BAP\", \"type\": \"ON-FULFILLMENT\", \"status\": \"NOT-PAID\", \
            \\"tags\": [{\"descriptor\": {\"code\": \"BUYER_FINDER_FEES\"}, \"list\": [{\"descriptor\": {\"code\": \"BUYER_FINDER_FEES_PERCENTAGE\"}, \"value\": \"5.0\"}]}]}" ::
              BL.ByteString
      let decoded = decode json :: Maybe Spec.Payment
      decoded `shouldSatisfy` isJust
      Spec.paymentCollectedBy (fromJust decoded) `shouldBe` Just "BAP"
      (Spec.paymentTags (fromJust decoded) >>= \xs -> Just (length xs)) `shouldBe` Just 1

    it "v2.1.0 payment with BAP_TERMS tag group in JSON" $ do
      let json =
            "{\"collected_by\": \"BAP\", \"type\": \"ON-FULFILLMENT\", \"status\": \"NOT-PAID\", \
            \\"tags\": [{\"descriptor\": {\"code\": \"BAP_TERMS\"}, \"list\": [{\"descriptor\": {\"code\": \"BUYER_FINDER_FEES_PERCENTAGE\"}, \"value\": \"5.0\"}]}]}" ::
              BL.ByteString
      let decoded = decode json :: Maybe Spec.Payment
      decoded `shouldSatisfy` isJust
      (Spec.paymentTags (fromJust decoded) >>= \xs -> Just (length xs)) `shouldBe` Just 1

  describe "golden: context version handling" $ do
    it "v2.0.0 context parses correctly" $ do
      let json =
            "{\"action\": \"search\", \"domain\": \"ONDC:TRV10\", \"version\": \"2.0.0\", \
            \\"bap_id\": \"bap.test.com\", \"bap_uri\": \"https://bap.test.com\", \
            \\"ttl\": \"PT30S\"}" ::
              BL.ByteString
      let decoded = decode json :: Maybe Spec.Context
      decoded `shouldSatisfy` isJust
      Spec.contextVersion (fromJust decoded) `shouldBe` Just "2.0.0"
      Spec.contextTtl (fromJust decoded) `shouldBe` Just "PT30S"

    it "v2.1.0 context parses correctly" $ do
      let json =
            "{\"action\": \"on_search\", \"domain\": \"ONDC:TRV10\", \"version\": \"2.1.0\", \
            \\"bpp_id\": \"bpp.test.com\", \"bpp_uri\": \"https://bpp.test.com\", \
            \\"ttl\": \"PT30S\"}" ::
              BL.ByteString
      let decoded = decode json :: Maybe Spec.Context
      decoded `shouldSatisfy` isJust
    ...
    it "context with both bap and bpp fields round-trips" $ do
      roundTrip sampleContext
